...
import { VIDEO_DETAILS, YOUTUBE_CHANNEL_API, YOUTUBE_VIDEO_API } from "../utils/constants";
...
const WatchPage = () => {
  ...
  if (error) {
    return <div className="flex justify-center items-center">Error: {error.message}</div>;
  }

  if (!video || !video.items || video.items.length === 0) {
    return <div className="flex justify-center items-center">No video details found.</div>;
  }
